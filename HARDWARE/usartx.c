...
返回  值：无
**************************************************************************/
static inline void usart1_send(u8 data)
{
	USART1->DR = data;
...
返回  值：无
**************************************************************************/
static inline void usart2_send(u8 data)
{
	USART2->DR = data;
...
返回  值：无
**************************************************************************/
static inline void usart3_send(u8 data)
{
	USART3->DR = data;
...
返回  值：无
**************************************************************************/
static inline void usart5_send(u8 data)
{
	UART5->DR = data;
...
/**
 * @brief  Transmits a string of data over the specified USART peripheral.
 * @param  USART: Pointer to the USART peripheral (e.g., USART1, USART2, USART3, UART5).
 *                 Must be a valid USART peripheral.
 * @param  str: Pointer to the data buffer to be transmitted.
 * @param  size: Size of the data buffer (number of bytes to transmit).
 * @param  Timeout: Timeout value for transmission (in arbitrary units).
 * @retval None
 * @note   This function transmits data byte by byte and waits for the transmission
 *         to complete for each byte. If the Timeout value reaches zero, the function
 *         exits without completing the transmission.
 * @note   The function performs basic parameter validation, including checking for
 *         NULL pointers, zero size, and invalid USART peripherals.
 */
void UART_Transmit(USART_TypeDef *USART, uint8_t *str, uint16_t size)
{
	uint16_t i;
	// Check if the USART pointer is valid
	if (USART == NULL || str == NULL || size == 0) {
		return; // Invalid parameters
	}
	if (USART != USART1 && USART != USART2 && USART != USART3 && USART != UART5) {
		return; // Invalid USART peripheral
	}
